/*
        ...
        you are free to use and modify this but PLEASE credit me (at least
   retain the name in some way)

        ** read all of the docs before using as they contain some important
   tidbits **
        ...
        **** INSTRUCTION SEMANTICS ****
        ...
        (instructions with a * change condition codes)
                ...
                *LEA: r[src] = pc
                ...
                TRAP (vector 2): r[7] = pc, pc = r[0], prints null terminated
   ascii string TRAP (vector 3): r[7] = pc, pc = r[0], gets user inputted string
   with max length stored in r[1] and stores it sequentially at address in r[0]

                ex. trap 0

        **** REGISTERS ****
        r0...r7: general purpose registers (r0 is usually used as an argument
   for trap calls, r7 is used for return addresses) accumulator: register inside
   alu that stores results of arithmetic and logical operations ir (instruction
   register): stores the current instruction the cpu is executing pc (program
   counter): stores the memory address of the NEXT instruction to execute (gets
   incremented during fetch phase of execution, so keep in mind anything using
   pc-relative addressing will be using the incremented pc) cc: 3 bit flags
   which consist of n (negative), z (zero), and p (positive), are changed
   whenever an instruction modifies a general purpose register

        **** NOTES ****
        any immediate values in assembly will clamp to be within bounds, e.g.
   for ADD normal immediate, if you put 60 as the immediate value it will clamp
   to 15 any unused bits in a machine code instruction will be set to 0 strings
   are NOT packed!!! pc is *always* incremented at the start of execution, so
   instructions that change or use pc such as jmp will use the incremented pc as
   the base sext stands for sign extend and not anything else you MUST include
   trap 0 to halt the program somewhere or else it'll crash condition codes are
   set after instructions with *, so if the result of an instruction with an *
   is: negative 	-> n bit is set zero 		-> z bit is set positive
   -> p bit is set
*/
...
int assemble() {
  ...
  while (fgets(line, 256, file)) {
    ...
    if (strcmp(tokens[0], ".fill") == 0) {
      ...
      instruction += ((conditionCodes & 0x7) << 9);
